...
#define PROID			"4300VXBSWr"

#define ACCESS_KEY		"OUxuNW50RGdqMDA3eENDR093NVl4dHNRc3Z2VDVBWGM="

#define DEVICE_NAME		"luo"
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temperature\":{\"value\":%d},", temp);
	strcat(buf, text);
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				str = req_payload;
				tmp = key;
				UsartPrintf(USART_DEBUG, "str: %s\r\n", str);
				while (*str != '\0') 
				{ 
//					UsartPrintf(USART_DEBUG, "once while-----------------\r\n");
//					UsartPrintf(USART_DEBUG, "*str: %c,%d, *tmp: %c,%d\r\n", *str, *str, *tmp, *tmp);
					if (*str == *tmp) 
					{
            int t = 1;
            while (*tmp != '\0') 
            ...
            if (*tmp == '\0' && t) 
						{
//							UsartPrintf(USART_DEBUG, "find led\r\n");
							// 说明找到led
							str++; str++;
							if (*str == 't') 
							{
...
//	led_json = cJSON_GetObjectItem(raw_json, "led");
